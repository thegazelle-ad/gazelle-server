...
export default class ArticleController extends FalcorController {
  static getFalcorPath(params) {
    return ['articles', parseInt(params.id), ["title", "body"]];
  }

  render() {
    console.log("RENDERING ARTICLE CONTROLLER");
    if (this.state.ready) {
      const articleData = this.state.data.articles[parseInt(this.props.params.id)];
      return (
        <div>
          <h2>Controller for article: {this.props.params.id}</h2>
