...
- [deployment-resources](../deployment-resources) [This is where we keep all the scripts and config needed for deployment]
  - [config](../deployment-resources/config) [This is where we at the time of writing keep our config for Caddy, the server that redirects all our traffic to our node servers]
  - [http-error-static-pages](../http-error-static-pages) [A generator of error html pages (and where those html pages will be kept) for our main web server Caddy to send if our node server experiences problems]
  - [scripts](../deployment-resources/config) [All the bash scripts needed for deployment]
